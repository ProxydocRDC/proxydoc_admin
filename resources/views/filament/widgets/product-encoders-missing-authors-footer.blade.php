{{-- @php
    ...
    $columnCount = is_countable($columns ?? null) ? count($columns) : 0;
    $metricCount = 4; // semaine, mois, période, total
    $labelColspan = max(1, $columnCount - $metricCount);
@endphp

@if(($missingAuthorsTotal ?? 0) > 0)
    <div class="mt-3">
        <x-filament-tables::table class="w-full">
            <tbody class="bg-white dark:bg-gray-900">
                <tr class="bg-gray-50 dark:bg-gray-800">
                    <td colspan="{{ $labelColspan }}" class="px-4 py-3 text-sm font-medium text-gray-700 dark:text-gray-200">
                        Auteur manquant
                    ...
                    <td class="px-4 py-3 text-sm text-center text-gray-700 dark:text-gray-300"></td>
                </tr>
            </tbody>
