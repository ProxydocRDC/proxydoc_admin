    ...
    WithChunkReading
{
    ...
    protected function normalizeCurrency($v): ?string
    {
        ...
        if (in_array($s, ['CDF','FC','CDFR','RDC','CDFONGO'])) return 'CDF';
        return null;
    }
    ...
    {
        if (blank($idOrSkuOrName)) return null;
    ...
    public function onRow(Row $row): void
    {
        ...
        $errors = [];
        if (empty($pharmacyId)) $errors['pharmacy'] = ['Pharmacie introuvable (pharmacy_id/pharmacy_name).'];
        if (empty($productId))  $errors['product']  = ['Produit introuvable (product_id/product_sku/product_name).'];

        $currency = $this->normalizeCurrency(Arr::get($r,'currency'));
        if (!$currency) $errors['currency'] = ['Devise invalide (USD/CDF attendue).'];

        if ($errors) {
            throw new \Maatwebsite\Excel\Validators\ValidationException(
                LvValidationException::withMessages($errors)
            );
        }
